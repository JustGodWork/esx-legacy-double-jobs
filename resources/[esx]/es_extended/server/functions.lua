...
function Core.SavePlayers(cb)
	...
	if count > 0 then
		...
		MySQL.prepare("UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `job2` = ?, `job2_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ? WHERE `identifier` = ?", parameters,
		function(results)
			if results then
...
function ESX.GetIdentifier(playerId)
	for _, v in ipairs(GetPlayerIdentifiers(playerId)) do
		if string.match(v, string.format("%s:", Config.identifier)) then
			local identifier = v
			return identifier
		end
	end
